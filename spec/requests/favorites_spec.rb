  ...
  context "お気に入り登録処理" do
    context "ログインしている場合" do
      ...
      it "料理のお気に入り登録ができること" do
        expect {
          post "/favorites/#{new_post.id}/create"
        }.to change(new_post.favorites, :count).by(1)
      end

      it "料理のAjaxによるお気に入り登録ができること" do
        expect {
          post "/favorites/#{new_post.id}/create", xhr: true
        }.to change(new_post.favorites, :count).by(1)
      end

      it "料理のお気に入り解除ができること" do
        user.favorite(new_post)
        expect {
          delete "/favorites/#{new_post.id}/destroy"
        }.to change(new_post.favorites, :count).by(-1)
      end
      ...
      it "料理のAjaxによるお気に入り解除ができること" do
        user.favorite(new_post)
        expect {
          delete "/favorites/#{new_post.id}/destroy", xhr: true
        }.to change(new_post.favorites, :count).by(-1)
      end
    end
